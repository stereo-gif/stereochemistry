...
# 2. تصميم الواجهة (نفس كودك بالظبط)
st.markdown("""
...
# 3. مدخلات المستخدم
compound_name = st.text_input("Enter Structure Name (e.g., 1,2-dichloroethene or 1,3-dichloropropadiene):", "")
    ...
    if not compound_name:
        ...
        try:
            ...
            if not results:
                ...
                # --- [بداية التعديل الخاص بالألين فقط] ---
                # البحث عن نظام الألين C=C=C وتنشيطه
                patt = Chem.MolFromSmarts("C=C=C")
                if mol.HasSubstructMatch(patt):
                    matches = mol.GetSubstructMatches(patt)
                    for match in matches:
                        # بنخلي الذرة اللي في النص ليها ChiralTag عشان الـ Enumerate يشوفها
                ...
                # خيار tryEmbedding مهم للألين عشان يتأكد إن الصور معكوسة فعلاً
                ...
                if len(isomers) <= 1:
                    st.info(f"✨ The compound {compound_name} is Achiral or has no extra isomers.")
                ...
                # رجعنا للرسم الأصلي GridImage
                img = Draw.MolsToGridImage(
                    isomers,
                    molsPerRow=3,
                    subImgSize=(350, 350),
                    legends=labels,
                    useSVG=True # SVG بيخلي خطوط الـ Allene (الـ Wedges) أوضح بكتير
                )
